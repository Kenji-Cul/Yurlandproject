...
<body class="profile-body">
  ...
  foreach ($allagents as $key => $value2) {
    $customer2 = $user ->selectAllEarningAgents($value2['uniqueagent_id']);
    $agentid = [];
    foreach ($customer2 as $key => $value) { 
        array_push($agentid,$value['agent_id']);
    }
  ...
  foreach ($allagents as $key => $value2) {
    ...
    ?>
                <div class="account-detail2">
                    <div class="radius">
                        <?php if(!empty($customer['agent_img'])){?>
                        <img src="profileimage/<?php echo $customer['agent_img'];?>" alt="profile image" />
                        <?php }?>
                        <?php if(empty($customer['agent_img'])){?>
                        <div class="empty-img">
                            <i class="ri-user-fill"></i>
                    ...
                    <div class="flex">
                        ...
                        <span class="email-span"><?php echo $customer['agent_email'];?></span>
                    </div>
                    <a href="agentinfo.php?unique=<?php echo $customer['uniqueagent_id'];?>&real=91838JDFOJOEI939"
                        style="color: #808080;"><i class="ri-arrow-right-s-line"></i></a>
                </div>

                <?php }}?>

                <?php if(empty($agentid2)){?>
                <div class="success">
                    <img src="images/asset_success.svg" alt="" />
                    <p>There are no earning agents yet!</p>
                </div>
                <?php }?>
    ...
    searchIcon.onclick = () => {
        ...
        searchinput2.onkeyup = () => {
            ...
            xhr.open("POST",
                `searchearningagent.php?user=supadmin`);
    ...
    function htmlTableToExcel(type) {
        ...
        XLSX.writeFile(file, 'Earningagentsdata.' + type);
    }

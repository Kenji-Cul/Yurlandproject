...
<body class="profile-body">
    ...
    <div class="flex-container">
       ...
       $user = new User;
       $total = $user->selectLandInfo3();
      if(!empty($total)){
      
       foreach ($total as $key => $value) {
        if($value['offline_status'] == "Pending"){
    ?>
                    <tr>
            ...
            <div class="navigation-div" style="margin-top: 2em;">
                <div class="navig">
                    ...
                    <div class="payment hide">Paid By</div>
                    <div class="payment hide">Payment Date</div>
    ...
    $user = new User;
           ...
           if(!empty($total)){
            ...
            foreach ($total as $key => $value) {
                 ?>
                <div class="transaction-details2" <?php if($value['offline_status'] == "Rejected"){?>
             ...
             ?></span></p>
                    ...
                    <div class="details hide flexdetail" style="text-transform: capitalize;">
                        <p class="pname email-span"> <?php echo $value['payee'];?></p>
                    </div>
                    ...
                    <?php  $name = $value['product_id'].$value['payment_id']; ?>

                    <div class="flexform"
                        style="display: flex; align-items: center; justify-content: center; flex-direction:column; gap:1em;">
                        <form action="" class="approve-form" method="POST">
                            <input class="price" type="submit" value="Approve" name="approve<?php echo $name?>"
                                style="background-color: green; color: #fff;  padding: 0;" />
                        </form>
                        <?php  
                    ...
                    if(isset($_POST["approve".$name])){ 
                                 ...
                                 $insertupdate = $user->updateLandHistory5($value['payment_id'],$value['product_id'],$value['payment_id'],$value['payment_method'],$value['newpay_id'],$approve,$updatedate);
    
                                 
                                 if($value['payment_method'] == "Outright"){
                                    $selectland = $user->selectPayment2($value['product_id'],$value['newpay_id'],$value['customer_id'],$value['payment_method']);
                                 foreach($selectland as $key => $value2){
                                    $price = $value2['balance'];
                                    $prodprice = $value2['product_price'];
                                    $insertupdate = $user->updatePayment3($value['product_id'],$value['newpay_id'],$value['customer_id'],$value['payment_method'],$price,$prodprice);
                                 }
                                 }

                                 if($value['payment_method'] == "NewPayment"){
                                    $selectland = $user->selectPayment2($value['product_id'],$value['newpay_id'],$value['customer_id'],$value['payment_method']);
                                 foreach($selectland as $key => $value2){
                                    $time = $value2['period_num'];
                                    $price = $value2['balance'];
                                    $prodprice = $value2['product_price'];
                                    $insertupdate = $user->updatePayment2($value['product_id'],$value['newpay_id'],$value['customer_id'],$value['payment_method'],$time,$price,$prodprice);
                                 }
                                 }
                                

                                 $updateearning = $user->updateEarningHistory2($value['newpay_id'],$approve,$updatedate);
                                
                                 $deletedp = "approvedhistory";
                                 header("Location: successpage/deletesuccess.php?detect=".$deletedp."");
                        } 
                        ?>

                        <form action="" class="reject-form" method="POST">
                            <input class="price" type="submit" value="Reject" name="reject<?php echo $name?>"
                                style="background-color: #7e252b; color: #fff;  padding: 0; " />
                        </form>
                        <?php  
                 
                    if(isset($_POST["reject".$name])){ 
                        $reject = "Rejected";
                        $updatedate = date("M-d-Y");
                                 $insertupdate = $user->updateLandHistory6($value['payment_id'],$value['product_id'],$value['payment_id'],$value['payment_method'],$value['newpay_id'],$reject,$updatedate);

                                 if($value['payment_method'] == "Outright"){
                                    $selectland = $user->selectPayment2($value['product_id'],$value['newpay_id'],$value['customer_id'],$value['payment_method']);
                                    foreach($selectland as $key => $value2){
                                       $price = $value2['balance'] + $value['product_price'];
                                       $prodprice = $value2['product_price'] - $value['product_price'];
                                       $insertupdate2 = $user->updatePayment3($value['product_id'],$value['newpay_id'],$value['customer_id'],$value['payment_method'],$price,$prodprice);
                                    }
                                 }
                                 ...
                                 if($value['payment_method'] == "NewPayment"){
                                    $selectland = $user->selectPayment2($value['product_id'],$value['newpay_id'],$value['customer_id'],$value['payment_method']);
                                    foreach($selectland as $key => $value2){
                                       $time = $value2['period_num'] + $value['period_num'];
                                       $price = $value2['balance'] + $value['product_price'];
                                       $prodprice = $value2['product_price'] - $value['product_price'];
                                       $insertupdate2 = $user->updatePayment2($value['product_id'],$value['newpay_id'],$value['customer_id'],$value['payment_method'],$time,$price,$prodprice);
                                    }
                                 }
                                 

                                 $updateearning = $user->updateEarningHistory2($value['newpay_id'],$reject,$updatedate);

                                 
                                
                                 $deletedp = "rejecthistory";
                                 header("Location: successpage/deletesuccess.php?detect=".$deletedp."");
                        } 
                        ?>
                    </div>
           ...
           ?>
                ...
                <div class="success">
                    ...
                    <p>There are no offline payments yet!</p>
                </div>
                <?php }?>
    ...
    function htmlTableToExcel(type) {
        ...
        XLSX.writeFile(file, 'offlinepayments.' + type);
    }
